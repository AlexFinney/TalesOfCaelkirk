...
public class ANewAdventureQuest extends Quest {
	...
	public void entityKilled(LivingDeathEvent e) {
		...
		if (e.getEntity() instanceof EntityGoblin && e.getSource().getTrueSource() instanceof PlayerEntity) {
			...
			ANewAdventureQuestProgress qp = QuestManager.getQuestProgressForPlayer(player.getUniqueID(),
					ANewAdventureQuestProgress.class);
			if(!qp.smithingStarted) return;
	...
	public void oakLogAdded(ItemAddedToInventoryEvent e) {
		...
		ANewAdventureQuestProgress qp = QuestManager.getQuestProgressForPlayer(e.getEntity().getUniqueID(),
				ANewAdventureQuestProgress.class);

		if (qp.ulricTalkedTo) {
			if (qp.logsChopped < 10) {
				Util.sendTaskUpdateMessage(player, this,
						String.format("Harvest Oak Logs (%s/%s)", ++qp.logsChopped, 10));
				if (qp.logsChopped == 10) {
	...
	public void itemCrafted(ItemCraftedEvent e) {
		...
		ANewAdventureQuestProgress qp = (ANewAdventureQuestProgress) QuestManager
				.getQuestProgressForPlayer(e.getPlayer().getUniqueID(), getQuestProgressClass());
		if (qp.craftingStarted && (qp.sticksCrafted < 3 || qp.stringCrafted < 2)) {
			if (e.getCrafting().getItem() == Items.STRING && qp.stringCrafted < 2) {
				qp.stringCrafted = Math.min(2, qp.stringCrafted + e.getCrafting().getCount());
				Util.sendTaskUpdateMessage(e.getPlayer(), QuestManager.A_NEW_ADVENTURE,
						String.format("Craft the components of a Fishing Rod: String (%s/2), Oak Sticks (%s/3)",
								qp.stringCrafted, qp.sticksCrafted));
				qp.save();
			} else if (e.getCrafting().getItem().equals(TOCItems.stick_oak) && qp.sticksCrafted < 3) {
				qp.sticksCrafted = Math.min(3, qp.sticksCrafted + e.getCrafting().getCount());
				Util.sendTaskUpdateMessage(e.getPlayer(), QuestManager.A_NEW_ADVENTURE,
						String.format("Craft the components of a Fishing Rod: String (%s/2), Oak Sticks (%s/3)",
								qp.stringCrafted, qp.sticksCrafted));
			...
			if (qp.sticksCrafted >= 3 && qp.stringCrafted >= 2) {
				Util.sendNewTaskMessage(e.getPlayer(), QuestManager.A_NEW_ADVENTURE,
						"Return to Eva Teffan with your crafting components.");
			}

		} else if (qp.craftingStarted && !qp.fishingRodCrafted) {
			if (e.getCrafting().getItem().equals(Items.FISHING_ROD)) {
				Util.sendTaskUpdateMessage(e.getPlayer(), QuestManager.A_NEW_ADVENTURE,
						"Craft a Fishing Rod. (Complete)");
				Util.sendNewTaskMessage(e.getPlayer(), QuestManager.A_NEW_ADVENTURE,
						"Return to Eva Teffan with your Fishing Rod.");
	...
	public void sheepSheared(SheepShearedEvent e) {
		if (e.getEntity().world.isRemote)
			return;
		ANewAdventureQuestProgress qp = QuestManager.getQuestProgressForPlayer(e.shearer.getUniqueID(),
				ANewAdventureQuestProgress.class);
	...
	public void blockMined(BlockMinedEvent e) {
		ANewAdventureQuestProgress qp = (ANewAdventureQuestProgress) QuestManager
				.getQuestProgressForPlayer(e.player.getUniqueID(), getQuestProgressClass());
		...
		if (qp.kelvinTalkedTo && !qp.kelvinFinished) 
		{
			...
			{
				if (++qp.copperMined < 5) {
					Util.sendTaskUpdateMessage(e.player, this,
							String.format("Mine Copper Ore: (%s/5)", qp.copperMined));
				} else {
					if (qp.copperMined == 5) {
						Util.sendTaskUpdateMessage(e.player, this, "Mine Copper Ore: Complete");
						if (qp.tinMined >= 5) {
							Util.sendNewTaskMessage(e.player, this,
									"Mining task complete. Return to Kelvin Whitestone");
						}
					}
				}
				qp.save();
	...
	public void bronzeSmithed(ItemSmithedEvent e) {
		...
		if (qp.bronzeSmelted < 5 && e.smithed.getItem() == TOCItems.ingot_bronze) {
			if (++qp.bronzeSmelted >= 5) {
				...
				Util.sendTaskUpdateMessage(e.player, this,
						String.format("Smelt 5 bronze ingots. (%s/5)", qp.bronzeSmelted));
			}
			qp.save();
		}
	...
	@Override
	public QuestProgress getNewQuestProgressInstance(UUID playerId) {
		return new ANewAdventureQuestProgress(playerId, id, 0);
	}
	...
	public static class ANewAdventureQuestProgress extends QuestProgress {
		...
		public int stringCrafted = 0;
		public int sticksCrafted = 0;
