...
public class HUD extends Gui {
	...
	void drawEffectIcons() {
		if(activeEffects.size() > 0) {
			...
			for(int i = 0; i < activeEffects.size(); ++i) {
				...
				ResourceLocation icon = new ResourceLocation(Reference.MODID, "textures/icons/effects/" + activeEffects.get(i) + ".png");
				tm.bindTexture(icon);
				drawScaledCustomSizeModalRect((int)(iconX - iconSize / 2f), (int)(y), 0, 0, iconSize, iconSize, iconSize, iconSize, iconSize, iconSize);
			}
	...
	void drawHealthBar(int curHealth, int maxhealth)
	{
		...
		drawScaledCustomSizeModalRect(barX, barY, 0, 0, barWidth, barHeight, (int)(barWidth * scale), barHeight, barWidth, barHeight);
		...
		drawString(Minecraft.getMinecraft().fontRenderer, s, barX + barWidth / 2 - 20,  barY + barHeight / 2 - 4, 0xFFFFFF);
		
		
		GlStateManager.scale(.75, .75, .75);
		drawString(Minecraft.getMinecraft().fontRenderer,  String.format("%.2f", TOCMain.localPlayer.getHealthRegen()), (int)((barX + barWidth - 23) * 1.333f),  (int)((barY + barHeight / 2 - 3) * 1.333f), 0xFFFFFF);
		...
		if(((EntityPlayer) TOCMain.localPlayer.mcEntity).getHeldItem(EnumHand.MAIN_HAND).getItem() instanceof ISpecialAttackWeapon) {
			GlStateManager.pushMatrix();
			GlStateManager.scale(.25, .25, .25);
			
			scale = .5f;
			String name = ((EntityPlayer) TOCMain.localPlayer.mcEntity).getHeldItem(EnumHand.MAIN_HAND).getItem().getRegistryName().toString();
			...
			tm.bindTexture(healthBar);
			drawScaledCustomSizeModalRect(barX*4 + barWidth * 4 - barWidth, barY*4 - barHeight, 0, 0, barWidth, barHeight, (int)(barWidth * scale), barHeight, barWidth, barHeight);
			
			if(scale == 1) {
				drawScaledCustomSizeModalRect(barX*4 + (int)(barWidth * 4.05f), barY*4 - barHeight, 0, 0, barWidth, barHeight, (int)(barWidth * .1f), barHeight, barWidth, barHeight);
			}
			
			tm.bindTexture(emptyBarHealth);
			drawModalRectWithCustomSizedTexture(barX*4 + barWidth * 4 - barWidth, barY*4 - barHeight, 0, 0, barWidth, barHeight, barWidth, barHeight);
	...
	void drawManaBar(int curMana, int maxMana)
	{
		...
		GlStateManager.scale(.75, .75, .75);
		drawString(Minecraft.getMinecraft().fontRenderer, String.format("%.2f", TOCMain.localPlayer.getManaRegen()), (int)((barX + barWidth - 23) * 1.333f),  (int)((barY + barHeight / 2 - 2) * 1.333f), 0xFFFFFF);
		GlStateManager.scale(1.333f, 1.333f, 1.333f);
	...
	void drawXpMessages() {
		...
		for(XpMessage msg : xpMessages) {
			...
			drawModalRectWithCustomSizedTexture(msg.x - iconDim, (int)(msg.y - iconDim / 4f), iconDim, iconDim, iconDim, iconDim, iconDim, iconDim);
