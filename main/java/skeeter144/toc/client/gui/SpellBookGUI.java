	...
	private void drawSpellIcons() {
		...
		for(Spell s : Spells.spells) {
			...
			if(!wasMouseClicked && Mouse.isButtonDown(0)) {
				...
				drawString(fr, str, adjustedX, adjustedY, 0xFFFFFFFF);
		...
		drawModalRectWithCustomSizedTexture(buttonX, buttonY, 0, 0, buttonWidth, buttonHeight, buttonWidth, buttonHeight);
		...
		if(embueBtn.contains(adjustedX, adjustedY)) {
			...
			if(!wasMouseClicked && Mouse.isButtonDown(0) && selectedIcon >= 0) {
				...
				if(held != null) {
					...
					Network.INSTANCE.sendToServer(new WandEmbueMessage(Minecraft.getMinecraft().player, hand, selectedIcon));
				}else {
					Minecraft.getMinecraft().player.sendChatMessage("You must be holding a wand to embue a spell onto it!");
				}
			}
		}
